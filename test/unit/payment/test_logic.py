...
class TestPaymentLogic(unittest.IsolatedAsyncioTestCase):
    ...
    @patch('payment.payment_logic.db')
    ...
    async def test_get_user_success(self, mock_db):
        ...
        self.assertEqual(expected_value.credit, actual.credit)
        self.assertIsNone(err)
    ...
    async def test_add_credit_success(self, mock_db):
        ...
        mock_db.get = AsyncMock(return_value=msgpack.encode(UserValue(credit=initial_value)))
        mock_db.set = AsyncMock(return_value=(user_id, None))

        result, err = await logic.add_credit(user_id, amount)

        self.assertEqual(initial_value + amount, result)
        self.assertIsNone(err)
        mock_db.set.assert_called_with(user_id, msgpack.encode(UserValue(credit=initial_value + amount)))
    ...
    async def test_remove_credit_success(self, mock_db):
        ...
        self.assertEqual(result, updated_value.credit)
        self.assertIsNone(err)
